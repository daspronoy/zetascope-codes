...
// Frame chunk size for shared memory tiling and FP16 partial accumulation (Steps 1+5)
// Accumulates FRAME_CHUNK_C frames in FP16 then adds FP32 partial sum to stat.
// Safe for 16 terms: max INT8 dequant value ≈ 1.0, 16 terms → sum ≤ 16.0 (well within FP16 range ~65504).
#define FRAME_CHUNK_C 16
...
// Step 1+5: frame-chunk loop accumulates FRAME_CHUNK_C frames
// in FP16 partial sums, then promotes to FP32 accumulator.
// FP16 has 2× throughput on sm_87+ accumulate path; 16-term
// chunks keep rounding error bounded (max sum ~16.0 << 65504).
// ============================================================
    ...
    int nx, int ny, int nf)
{
    ...
    for (int t_base = 0; t_base < nf; t_base += FRAME_CHUNK_C) {
        __half partial = __float2half(0.0f);
        const int t_end = (t_base + FRAME_CHUNK_C < nf) ? t_base + FRAME_CHUNK_C : nf;
        ...
        for (int t = t_base; t < t_end; t++) {
            ...
            const float val = (float)__ldg(&frames[(size_t)t * frame_stride + py * nx + px])
                              * g_frame_scale[t];
            partial = __hadd(partial, __float2half(val));
        }
        stat += __half2float(partial);
    }
    return stat;
}
...
// Step 1+5: accumulates in FRAME_CHUNK_C-frame FP16 chunks then
//           promotes to FP32, matching the L1/L2 accumulation path.
// Step 3: load is coalesced (adjacent tx → adjacent columns); verify
//         with: ncu --metrics l1tex__t_sectors_pipe_lsu_mem_global_op_ld.sum,
//               l1tex__t_requests_pipe_lsu_mem_global_op_ld.sum
//               to confirm sectors/requests ratio ≈ 1.
// ============================================================
    ...
    int nx, int ny, int nf)
{
    ...
    for (int t_base = 0; t_base < nf && !out_of_bounds; t_base += FRAME_CHUNK_C) {
        __half partial = __float2half(0.0f);
        const int t_end = (t_base + FRAME_CHUNK_C < nf) ? t_base + FRAME_CHUNK_C : nf;
        for (int t = t_base; t < t_end; t++) {
            int px = __float2int_rn(x + vx * t);
            int py = __float2int_rn(y + vy * t);
            if (px < 0 || px >= nx || py < 0 || py >= ny) { out_of_bounds = true; break; }
            partial = __hadd(partial, __float2half(
                (float)__ldg(&frames[(size_t)t * frame_stride + py * nx + px])
                * g_frame_scale[t]));
        }
        if (!out_of_bounds) acc += __half2float(partial);
    }
    H[y * nx + x] = out_of_bounds ? -FLT_MAX : acc;
}
...
int main(int argc, char** argv) {
    ...
    // Step 6: Stream pipelining — pinned host memory + per-frame cudaMemcpyAsync.
    // CPU converts frame t+1 on the host while the GPU DMA engine transfers frame t
    // over PCIe (or to NVMM on Orin). Eliminates the ~5.70 s conversion + memcpy
    // serialisation bottleneck for large frame stacks.
    printf("Converting FP32 -> INT8 (per-frame scale, pipelined upload)...\n");
    auto up_t0 = std::chrono::steady_clock::now();

    size_t n_total = (size_t)n_pixels * nf;
    float* h_frame_scales = (float*)malloc(nf * sizeof(float));

    // Pinned host memory → DMA engine can transfer directly without a staging copy.
    uint8_t* h_frames_u8;
    CUDA_CHECK(cudaHostAlloc(&h_frames_u8, n_total, cudaHostAllocDefault));

    cudaStream_t upload_stream;
    CUDA_CHECK(cudaStreamCreate(&upload_stream));

    for (int t = 0; t < nf; t++) {
        // CPU: convert frame t on host (runs concurrently with GPU DMA of frame t-1)
        const float* src = h_frames_fp32 + (size_t)t * n_pixels;
        float max_val = 1e-30f;
        for (int i = 0; i < n_pixels; i++) max_val = fmaxf(max_val, src[i]);
        ...
        // GPU DMA: enqueue async transfer for this frame.
        // The DMA runs in the background while the next iteration converts frame t+1.
        CUDA_CHECK(cudaMemcpyAsync(d_frames + (size_t)t * n_pixels, dst,
                                   (size_t)n_pixels * sizeof(uint8_t),
                                   cudaMemcpyHostToDevice, upload_stream));
    }
    // Wait for all async frame uploads to finish
    CUDA_CHECK(cudaStreamSynchronize(upload_stream));
    CUDA_CHECK(cudaStreamDestroy(upload_stream));

    // Upload per-frame dequantisation scales
    CUDA_CHECK(cudaMemcpy(d_scale_arr, h_frame_scales, nf * sizeof(float),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpyToSymbol(g_frame_scale, &d_scale_arr, sizeof(float*)));

    CUDA_CHECK(cudaFreeHost(h_frames_u8));
    ...
    // Step 4: L2 Persistence — pin frame data in L2 cache across repeated kernel launches.
    // With Orin's 4 MB L2 (RTX 4090: 72 MB), this avoids re-fetching the same frame data
    // from DRAM on each of the nv0² shift_and_accumulate launches.
    cudaStream_t compute_stream;
    CUDA_CHECK(cudaStreamCreate(&compute_stream));
    {
        size_t max_win = 0;
        cudaDeviceGetAttribute(reinterpret_cast<int*>(&max_win),
                               cudaDevAttrMaxAccessPolicyWindowSize, 0);
        size_t win_bytes = (frame_bytes < max_win) ? frame_bytes : max_win;
        if (win_bytes > 0 && prop.major >= 8) {   // Ampere (sm_80+) required
            cudaStreamAttrValue attr = {};
            attr.accessPolicyWindow.base_ptr  = reinterpret_cast<void*>(d_frames);
            attr.accessPolicyWindow.num_bytes  = win_bytes;
            attr.accessPolicyWindow.hitRatio   = 1.0f;
            attr.accessPolicyWindow.hitProp    = cudaAccessPropertyPersisting;
            attr.accessPolicyWindow.missProp   = cudaAccessPropertyStreaming;
            CUDA_CHECK(cudaStreamSetAttribute(compute_stream,
                cudaStreamAttributeAccessPolicyWindow, &attr));
            printf("L2 persistence: %.2f MB of frame data pinned in L2 (window=%.2f MB)\n\n",
                   frame_bytes / 1e6, win_bytes / 1e6);
        }
    }
    ...
    printf("  [L0] Coarse search (shift-and-stack, %d x %d = %lld pairs)...\n",
           nv0, nv0, evals_L0);
    CUDA_CHECK(cudaEventRecord(t0, compute_stream));

    {
        int init_n = n_pixels * TOP_K;
        int init_block = 256;
        int init_grid  = (init_n + init_block - 1) / init_block;
        init_topk<<<init_grid, init_block, 0, compute_stream>>>(d_topk_L0, init_n);
...

    {
        ...
        for (int vi = 0; vi < nv0; vi++) {
            for (int vj = 0; vj < nv0; vj++) {
                shift_and_accumulate<<<sas_grid, sas_block, 0, compute_stream>>>(
                    d_frames, d_H, h_vel_L0[vi], h_vel_L0[vj], nx, ny, nf);
                topk_update_L0<<<upd_grid, upd_block, 0, compute_stream>>>(
    ...
    CUDA_CHECK(cudaEventRecord(t1, compute_stream));
    ...
    CUDA_CHECK(cudaEventRecord(t0, compute_stream));
    tbd_warp_L1<<<n_warp_blocks, block, 0, compute_stream>>>(d_frames, d_topk_L0, d_topk_L1);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(t1, compute_stream));
    CUDA_CHECK(cudaEventSynchronize(t1));
    float l1_ms;
    ...
    CUDA_CHECK(cudaEventRecord(t0, compute_stream));
    tbd_warp_L2<<<n_warp_blocks, block, 0, compute_stream>>>(
    ...
    reduce_argmax<<<red_grid, red_block, 0, compute_stream>>>(
